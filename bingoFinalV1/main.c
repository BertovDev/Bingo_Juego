...
//PRE: NO TIENE.
//POSTCONDICONES:
int cantarColumna(struct Jugador jugador1 , int cant);
//PRE: NO TIENE.
//POSTCONDICONES
int cantarLinea(struct Jugador jugador1 , int cant);
...
void comienzoDelJuego(struct Jugador jugador1 , struct JugadorPc jugadorPc2 ){
    ...
    int contadorColumnaJ = 0;
    int contadorLineaJ = 0;
    int salida = -1;
    int cant = jugador1.numDeCartones;
    printf("CANT : %d\n",cant);
    int i = 0;
    ...
    do{
        ...
        contadorColumnaJ = cantarColumna(jugador1 , cant);
        contadorLineaJ = cantarLinea(jugador1 , cant);
        i++;
        if(contadorBingoJugador == 15){
            printf("Felicidades GANASTE !\n");
            colPuntos = colPuntos + 70;
            for(int e = 0;e<contadorColumnaJ;e++){
                colPuntos = colPuntos + 10;
            }
            for(int e = 0;e<contadorLineaJ;e++){
                lineaPuntos = lineaPuntos + 20;
            }
            jugador1.puntos = jugador1.puntos + (((colPuntos + lineaPuntos)*contadorBolillas));
...
int cantarLinea(struct Jugador jugador1 , int cant){
    ...
    for(int i = 0;i<cant;i++){
        for(int r=0;r<RENGLON;r++){
            if((jugador1.vCarton[i].carton[r][0] == -1) && (jugador1.vCarton[i].carton[r][1] == -1) && (jugador1.vCarton[i].carton[r][2] == -1) && (jugador1.vCarton[i].carton[r][3] == -1) && (jugador1.vCarton[i].carton[r][4] == -1) && ((jugador1.fila[r])) == 0){
                jugador1.fila[r] = -1;
                contadorLinea = contadorLinea + 1;
                printf("FELICIDADES COMPLETASTE UNA FILA!!\n");
